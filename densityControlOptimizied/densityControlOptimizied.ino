...
void loop() {
  ...
  if (fairShare1 == fairShare2 && fairShare2 == fairShare3) {
    ...
    addToQueue(fairShare1, fairShare2, fairShare3);
    serviceQueue();
  }
}
...
} else if (fairShare3 >= fairShare2 && fairShare2 >= fairShare1) {
...
while (!myQueue.isEmpty()) {
  float fairShare = myQueue.Front();

  if (fairShare > 0) {
    int lane[3];
    if (fairShare == fairShare1) {
      lane[0] = redPin1;
      lane[1] = yellowPin1;
      lane[2] = greenPin1;
    } else if (fairShare == fairShare2) {
      lane[0] = redPin2;
      lane[1] = yellowPin2;
      lane[2] = greenPin2;
    } else {
    ...
    digitalWrite(lane[0], HIGH);
    delay(2000);
    digitalWrite(lane[0], LOW);

    delay(1000);
    digitalWrite(lane[1], HIGH);
    delay(1000);
    digitalWrite(lane[1], LOW);
